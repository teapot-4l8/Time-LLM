...
print(f'Plotting {num_samples} individual samples...')

# Plot 1: Multiple samples - Scaled
print('\n[1/7] Plotting scaled predictions for multiple samples...')
...
print('[2/7] Plotting original scale predictions for multiple samples...')
fig, axes = plt.subplots(num_samples, 1, figsize=(15, 3*num_samples))
if num_samples == 1:
    axes = [axes]
...
print('[3/7] Plotting side-by-side comparison...')
...
print('[4/7] Plotting error distributions...')
...
# Plot 5: Continuous time series (FIXED!)
print(f'[5/7] Plotting continuous time series ({args.continuous_steps} steps)...')

# For multivariate data, we have predictions organized as:
# Shape: (num_samples, pred_len) where num_samples = actual_time_samples
# Since we already extracted OT, we can directly use these predictions

# Calculate how many continuous steps we can show
num_samples_available = len(preds_scaled_ot)
max_continuous_samples = min(args.continuous_steps, num_samples_available)

if max_continuous_samples > 0:
    print(f'  Using {max_continuous_samples} consecutive prediction windows')

    # Method 1: Show overlapping predictions (more realistic)
    # Each prediction is 6 steps, and they overlap
    # We'll show the FIRST step of each prediction to create a continuous line

    # Take first step from each consecutive prediction window
    continuous_pred_scaled = preds_scaled_ot[:max_continuous_samples, 0]  # First step only
    continuous_true_scaled = trues_scaled_ot[:max_continuous_samples, 0]
    continuous_pred_original = preds_original_ot[:max_continuous_samples, 0]
    continuous_true_original = trues_original_ot[:max_continuous_samples, 0]
    ...
    axes[0].set_title(f'{args.target} Continuous Prediction (1-step ahead) - Scaled Data\n({len(continuous_pred_scaled)} consecutive windows, each predicting 1 step ahead)',
                     fontsize=14, fontweight='bold', pad=10)
    axes[0].set_xlabel('Time Step (prediction windows)', fontsize=12)
    ...
    axes[1].fill_between(time_axis,
                         ...
                         alpha=0.2, color='gray', label='Prediction Error')
    axes[1].set_title(f'{args.target} Continuous Prediction (1-step ahead) - Original Scale\n({len(continuous_pred_original)} consecutive windows)',
                     fontsize=14, fontweight='bold', pad=10)
    axes[1].set_xlabel('Time Step (prediction windows)', fontsize=12)
    ...
    # Additional plot: Show ALL 6 steps for fewer samples to see overlap
    print(f'  Creating overlapping multi-step view...')
    num_overlap_samples = min(30, num_samples_available)  # Show 30 windows

    fig, axes = plt.subplots(2, 1, figsize=(20, 10))

    # Prepare data for overlapping visualization
    for i in range(num_overlap_samples):
        # Each prediction window starts at a different time
        time_offset = i
        time_steps = time_offset + np.arange(args.pred_len)

        # Scaled
        if i == 0:
            axes[0].plot(time_steps, trues_scaled_ot[i, :],
                        color='#2E86AB', alpha=0.3, linewidth=1, label='Actual')
            axes[0].plot(time_steps, preds_scaled_ot[i, :],
                        color='#A23B72', alpha=0.3, linewidth=1, linestyle='--', label='Predicted')
        else:
            axes[0].plot(time_steps, trues_scaled_ot[i, :],
                        color='#2E86AB', alpha=0.3, linewidth=1)
            axes[0].plot(time_steps, preds_scaled_ot[i, :],
                        color='#A23B72', alpha=0.3, linewidth=1, linestyle='--')

        # Original
        if i == 0:
            axes[1].plot(time_steps, trues_original_ot[i, :],
                        color='#18A558', alpha=0.3, linewidth=1, label='Actual')
            axes[1].plot(time_steps, preds_original_ot[i, :],
                        color='#F18F01', alpha=0.3, linewidth=1, linestyle='--', label='Predicted')
        else:
            axes[1].plot(time_steps, trues_original_ot[i, :],
                        color='#18A558', alpha=0.3, linewidth=1)
            axes[1].plot(time_steps, preds_original_ot[i, :],
                        color='#F18F01', alpha=0.3, linewidth=1, linestyle='--')

    axes[0].set_title(f'{args.target} Overlapping {args.pred_len}-Step Predictions - Scaled Data\n(Showing {num_overlap_samples} overlapping prediction windows)',
                     fontsize=14, fontweight='bold', pad=10)
    ...
    axes[1].set_title(f'{args.target} Overlapping {args.pred_len}-Step Predictions - Original Scale\n(Showing {num_overlap_samples} overlapping prediction windows)',
                     fontsize=14, fontweight='bold', pad=10)
    ...
    plt.tight_layout()
    overlap_path = os.path.join(folder_path, f'{args.data}_{args.model_id}_OT_overlapping.png')
...
print(f'[6/7] Plotting overlapping prediction windows...')

# Show 3 consecutive predictions with all their steps
num_detailed = min(3, len(preds_scaled_ot))

print(f'[7/7] Plotting detailed multi-step predictions...')
