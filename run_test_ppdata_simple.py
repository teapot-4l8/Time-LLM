...
parser = argparse.ArgumentParser(description='Time-LLM Testing for PPData (Simplified)')
...
parser.add_argument('--load_results', type=str, default=None, help='path to load saved test results (skip inference)')
parser.add_argument('--save_results', action='store_true', default=True, help='save test results for future use')
...
result_file = os.path.join(folder_path, f'{args.data}_{args.model_id}_{args.pred_len}_inference.npz')
...
if args.load_results and os.path.exists(args.load_results):
    ...
    # We still need test_data for inverse transform
...
if not skip_inference:
    ...
    # Move model to device and convert to bfloat16 if using GPU
    ...
    # Save raw inference results if requested
...
# For multivariate forecasting (M), reshape data to group features together
# The data loader returns (num_samples * num_features, pred_len, 1)
# We need to reshape to (num_samples, pred_len, num_features)
if args.features == 'M':
    num_features = args.c_out
    total_samples = preds_scaled.shape[0]

    # Check if the data size is correct
    # total_samples should be divisible by num_features
    if total_samples % num_features != 0:
        print(f'\nWARNING: Total samples ({total_samples}) is not divisible by num_features ({num_features})')
        print(f'Adjusting to nearest valid size...')
    ...
    # Reshape from (num_samples * num_features, pred_len, 1) to (num_features, num_samples, pred_len, 1)
    # then squeeze and transpose to get (num_samples, pred_len, num_features)
    preds_scaled = preds_scaled.squeeze(-1)  # (total_samples, pred_len)
    trues_scaled = trues_scaled.squeeze(-1)  # (total_samples, pred_len)

    # Reshape to (num_features, num_samples, pred_len)
    ...
    # Transpose to (num_samples, pred_len, num_features)
...
final_result_file = os.path.join(folder_path, f'{args.data}_{args.model_id}_{args.pred_len}_results.npz')
...
print(f'\n✓ Final results saved to {final_result_file}')
print(f'  - To reload inference results next time, use: --load_results {result_file}')
...
print(f'Plotting {num_samples} samples with {num_features} features...')
...
print('[2/6] Plotting original scale predictions for multiple features...')
fig, axes = plt.subplots(num_features, 1, figsize=(15, 4*num_features))
if num_features == 1:
    axes = [axes]
...
print(f'  ✓ Saved: {original_features_path}')
plt.close()

# Plot 3: First feature - multiple samples (Scaled)
print('[3/6] Plotting scaled predictions for multiple samples (Feature 0)...')
...
print(f'  ✓ Saved: {scaled_samples_path}')
plt.close()

# Plot 4: First feature - multiple samples (Original scale)
print('[4/6] Plotting original scale predictions for multiple samples (Feature 0)...')
fig, axes = plt.subplots(num_samples, 1, figsize=(15, 4*num_samples))
if num_samples == 1:
    axes = [axes]
...
print(f'  ✓ Saved: {original_samples_path}')
plt.close()

# Plot 5: Side-by-side comparison for first sample and first feature
print('[5/6] Plotting side-by-side comparison...')
...
print('[6/6] Plotting error distributions...')
